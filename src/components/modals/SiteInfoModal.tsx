...
function Guide() {
  const items: [string, string][] = [
    ['今日看板', '管自选：管理自选增删改，"发现股票"可按板块、主题找标的一键加入，结果带一句话业务介绍。看诊断：律动分数 + 大白话状态（涨太猛了 / 高位稳着涨 / 横盘波动 / 跌不动了 / 还在往下跌 / 跌过头了）。价格走势支持 K 线 / 收盘线切换；筹码分布与资金流向是日线估算，仅供参考。"判断复盘"里能看到历史信号的命中率，自己验准不准。'],
    ['持仓', '手动录入股数和成本，看实时盈亏；按住手柄拖动排序；点任意一行跳到今日页看它的律动诊断。"本周关注"每周一清零、最多 6 只，自选里没有也能手动加，第一次加会问本周预算。"真实成本试算"算一来一回的佣金、平台费、换汇和保本价。'],
    ['我的持仓故事', '持仓行点 📖 展开：顶部是一句话公司介绍，下面是最近 6 笔买卖流水（买入带当天律动快照）、你的专属复盘（卖飞 / 买高次数，样本不足 3 笔显示"养成中"）。没记录时点"去记忆页记一笔"可直接补。'],
    ['操作记忆', '记下你的买卖操作和当时想法。直接问"今天可以卖 IBM 吗""想买点什么"，会结合律动给建议；闲聊会引导你记一笔。说错了可以改（"刚才那笔单价说错了是 227.92"），"有没有记重"可以查重复。攒够样本后，这里会变成你的专属复盘。'],
    ['资讯', '盘前瞻、盘后总结每天两报；今日大事看美股 / 国内快讯，财经日历看 FOMC、CPI、财报日。朋友圈发买入逻辑、避坑经验，顺手给朋友点赞。登录后可用。'],
    ['娱乐', '追高模拟器、消息面陷阱、定投 vs 梭哈、飞镖选股、高管打地鼠、K 线盲盒、股票大百科……玩的是心态，练的是纪律。积分可以"落袋为安"，登录后多设备同步；游戏许愿池可以点菜新游戏。'],
    ['顶栏', '右上角"中 | EN"一键切换中英双语（先行覆盖顶栏、底栏、登录弹窗，各页正文陆续跟上）。'],
  ];
  return (
    <div>
      {items.map(([t, d]) => (
...
function Legal() {
  return (
    <div>
      ...
      <p className={p}>有问题或想法，去"资讯"页的朋友圈留一句，站长看得到。</p>
    </div>
  );
}
...
function Timeline() {
  const events: [string, string, string][] = [
    ...
    ['2026-09-24', '娱乐再加码', '高管打地鼠、飞镖选股上线；股票大百科 12 主题 152 题开刷；游戏许愿池开张，每小时巡检新留言。'],
    ['2026-09-24', '记忆页更可靠', '中文数字识别（"二百三十五块"不再瞎）、说错可改（"刚才那笔单价说错了"）、记一笔 15 分钟防重复、"有没有记重"查重复、"同步到持仓"防连点。'],
    ['2026-09-24', '持仓三期：从记账到懂你', '本周关注（每周一清零、最多 6 只、自选没有也能手动加、按预算给集中度提示）；"我的持仓故事"（操作流水 + 专属复盘 + 一句话公司介绍，无记录可一键去记忆页补）；232 只股票一句话业务库，发现页与推荐同步展示。'],
    ['2026-09-24', '页头改版与双语开工', '去掉"安心享受生活"小 pill，标题单行、两句 tagline 移到右边；登录右侧加"中 | EN"切换（P0 双语开工，先覆盖顶栏、底栏、登录弹窗）。'],
  ];
  return (
