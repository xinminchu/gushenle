...
/** 二分测试9：模拟真实 AccuracyPanel 的 hooks 结构（loading → 数据切换），但不用 fetch，用 setTimeout 模拟异步 */
const FULL_DATA = {
  available: true,
  stats: {
    total: 173,
...
export default function TestAccHooksInner() {
  const [data, setData] = useState<typeof FULL_DATA | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    // 模拟 fetch 的异步，但不用真正的 fetch
    const t = setTimeout(() => {
      if (!cancelled) {
        setData(FULL_DATA);
        setLoading(false);
  ...
  if (loading) {
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', padding: 16, background: '#0f172a', minHeight: '100vh', color: '#e2e8f0' }}>
        <h1 style={{ fontSize: 16, fontWeight: 700, marginBottom: 12 }}>二分测试9：hooks 状态切换</h1>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1 text-xs space-y-1.5">
            <div className="text-slate-400">{stats.total} 个信号 · {stats.sampleDays} 天样本</div>
        ...
        <p className="mt-3 text-[10px] text-slate-600 leading-relaxed">命中规则：{FULL_DATA.rule}</p>
      </div>
      <p style={{ fontSize: 12, color: '#64748b', marginTop: 12 }}>
        如果闪退，问题在 hooks/状态切换；如果正常，问题在真实的 fetch（r.json()）。
      </p>
    </div>
  );
}
